var UsersService = {
        ...
        for(let i = 0; i < data.length; i++){
          ...
          <div class="col-lg-3">
            <div class="card" style="background-color:`+data[i].color+`">
              ...
              <div class="card-body">
                <h5 class="card-title">`+ data[i].user_name +`</h5>
                <p class="card-text">`+ data[i].email +`</p>
                <div class="btn-group" role="group">
                  <button type="button" class="btn btn-primary note-button" onclick="UsersService.get(`+data[i].id+`)">Edit</button>
                  <button type="button" class="btn btn-danger note-button" onclick="UseresService.delete(`+data[i].id+`)">Delete</button>
                </div>
              </div>
            </div>
          </div>
  ...
  add: function(note){
        ...
        update: function(){
          ...
          users.description = $('#user_name').val();
          users.created = $('#email').val();
